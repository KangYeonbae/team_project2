...
async function varifyID(username, password) {
    ...
    try {
        ...
        const result = await connection.execute(
            'SELECT * FROM team_users WHERE username = :username AND password = :password',
            { username, password }
        );
...
router.post('/',  bodyParser.urlencoded({ extended: false }), async (req, res) => {
    ...
    if (authenticatedUser) {
        ...
        // res.redirect(`/boardLJW?id=${authenticatedUser.id}&username=${authenticatedUser.username}&name=${authenticatedUser.name}`);
