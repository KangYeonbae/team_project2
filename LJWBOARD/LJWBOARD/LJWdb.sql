...
CREATE TABLE requirement_posts (
                       ...
                       content CLOB,
                       FILE_ORIGINAL_NAME VARCHAR2 (4000BYTE),
                       FILE_STORED_NAME VARCHAR2 (4000BYTE),
...
--  회원가입을 위한 시퀀스+트리거 생성
CREATE SEQUENCE team_users_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER team_users_bir
BEFORE INSERT ON team_users
FOR EACH ROW
BEGIN
SELECT team_users_seq.NEXTVAL INTO :new.id FROM dual;
END;
