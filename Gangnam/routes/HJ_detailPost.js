...
router.get('/:id', async (req, res) => {
    ...
    console.log('get :id'+ req.body.post_id);
    postId = req.body.post_id ? req.body.post_id: req.params.id ;
    ...
    try {
        ...
        // 조회수 증가 처리
        await conn.execute(
            `UPDATE Team1_posts SET views = views + 1 WHERE id = :id`,
            [postId]
        );
        ...
        const postResult = await conn.execute(
            `SELECT p.id, p.title, u.username AS author, p.content, TO_CHAR(p.created_at, 'YYYY-MM-DD') AS created_at,
             ...
             FROM Team1_posts p
                      JOIN Team1 u ON p.author_id = u.id
             WHERE p.id = :id`,
            [postId],
        ...
        const commentResult = await conn.execute(
            ...
            FROM Team1_comments c
            JOIN Team1 u ON c.author_id = u.id
            WHERE c.post_id = :id
            ORDER BY c.id`,
            [postId],
            { fetchInfo: { CONTENT: { type: oracledb.STRING } } }
