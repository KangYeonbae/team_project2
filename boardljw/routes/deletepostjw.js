...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        await conn.execute(
            `DELETE FROM requirement_posts WHERE post_id = :postId or parent_comment_id in (select id from requirement_comments where post_id = :postid) = :postId`,
            [ postId, postId ]
        );
        ...
        await conn.execute(
            'DELETE FROM requirement_posts WHERE id = :postId',
            [postId]
        );
