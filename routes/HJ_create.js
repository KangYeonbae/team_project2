...
router.post('/', upload.array('files', 5), async (req, res) => {
    ...
    try {
        ...
        const result = await conn.execute(
            `SELECT Team1_post_id_seq.NEXTVAL FROM DUAL`
        );
        const postId = result.rows[0][0];

        // 게시글 삽입
        await conn.execute(
            `INSERT INTO Team1_posts (id, author_id, title, content, file_original_name, file_stored_name) VALUES (:id, :authorId, :title, :content, :file_original_name, :file_stored_name)`,
            {
                id: postId,
                authorId: authorId,
                title: title,
