...
비전공자들로만 이루어진 팀인만큼 빠른회의를 통하여 보다 빨리 코드작성에 들어갔습니다.

다만 좀더 디테일하게 자료들이 들어가야하기에
큼직한 도시들에서 서울시로 지역 변동이 되었습니다.
<hr>

# 배포환경 바로가기 :  http://54.82.4.76:3000/main
...
### 테이블구조
![table](readmeimg/table.png)

>> 테이블구조입니다.
>> users 를 기준으로 각 팀원들이 맡은 게시판의 table 들이 나누어져있습니다.
>> 모두 테이블을 만드는것 먼저 시작하였기때문에 이부분에 있어서 이해도가 높습니다.



### 라우팅처리
![router](readmeimg/라우팅.jpg)

>> 라우팅 처리결과입니다.
>> 각 팀원별 스킬업이 우선되는 프로젝트였기때문에 모두한명씩 게시판을 제작하였고
>> 따라서 4개의 게시판이있기때문에 라우팅의 양도 많았습니다.
...
이지우 - 회원가입 기능 완료
<br><br>
이은진 - 명동역 커뮤니티 게시판 구현.<br>
카테고리(주제)를 이용한 명소, 맛집, 숙소로 구분할 수 있도록 글쓰기 기능을 추가하도록 구현하였으며
검색 기능에서도 주제 별로 검색할 수 있도록 했습니다.<br>
다만 현재 상황은 이미지 업로드(업로드 자체는 가능합니다.)의 자연스러운 현상 및 댓글의 답글 및 수정, 삭제 아직 구현이 되지 않아 댓글 관련된 부가 기능의 원활이 이루어지지 않습니다. 현재 이미지 업로드에 관한 수정 및 댓글의 답글 기능을 구현하도록 작업 중입니다.

배호진 - 현재 댓글에 답글 달기는 구현중에 있으며, 추후에 완성되면 업데이트 하겠습니다


강연배 - 모든 팀원들의 프로젝트 파일들을 라우팅완료하였고,
마지막으로 실데이터값을 이용한 동적 그래프를 완성하였습니다.

오늘로 발표를하고 팀프로젝트가 마무리 되어집니다. 모두 고생하셨습니다!
