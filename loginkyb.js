...
// 라우트 정의
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'login_kyb.html')); // Send the HTML file
});
...
app.post('/login_kyb', (req, res) => {
    ...
    if (username === 'kyb' && password === '1111') {
        res.render('dashboard', { username });
    } else {
        res.sendFile(path.join(__dirname, 'public', 'login_kyb.html'), { error: 'Invalid username or password' });
    }
});
